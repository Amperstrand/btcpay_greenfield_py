from http import HTTPStatus
from typing import Any, cast
from urllib.parse import quote

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.lightning_payment_data import LightningPaymentData
from ...types import Response


def _safe_from_dict(parser, response):
    if not response.content:
        return None
    try:
        return parser(response.json())
    except Exception:
        return None


def _get_kwargs(
    store_id: str,
    crypto_code: str,
    payment_hash: str,
) -> dict[str, Any]:

    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": "/api/v1/stores/{store_id}/lightning/{crypto_code}/payments/{payment_hash}".format(
            store_id=quote(str(store_id), safe=""),
            crypto_code=quote(str(crypto_code), safe=""),
            payment_hash=quote(str(payment_hash), safe=""),
        ),
    }

    return _kwargs


def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Any | LightningPaymentData | None:
    if response.status_code == 200:
        response_200 = _safe_from_dict(LightningPaymentData.from_dict, response)

        return response_200

    if response.status_code == 404:
        response_404 = cast(Any, None)
        return response_404

    if response.status_code == 503:
        response_503 = cast(Any, None)
        return response_503

    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[Any | LightningPaymentData]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    store_id: str,
    crypto_code: str,
    payment_hash: str,
    *,
    client: AuthenticatedClient,
) -> Response[Any | LightningPaymentData]:
    """Get payment

     View information about the requested lightning payment

    Args:
        store_id (str):
        crypto_code (str):
        payment_hash (str):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Any | LightningPaymentData]
    """

    kwargs = _get_kwargs(
        store_id=store_id,
        crypto_code=crypto_code,
        payment_hash=payment_hash,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    store_id: str,
    crypto_code: str,
    payment_hash: str,
    *,
    client: AuthenticatedClient,
) -> Any | LightningPaymentData | None:
    """Get payment

     View information about the requested lightning payment

    Args:
        store_id (str):
        crypto_code (str):
        payment_hash (str):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Any | LightningPaymentData
    """

    return sync_detailed(
        store_id=store_id,
        crypto_code=crypto_code,
        payment_hash=payment_hash,
        client=client,
    ).parsed


async def asyncio_detailed(
    store_id: str,
    crypto_code: str,
    payment_hash: str,
    *,
    client: AuthenticatedClient,
) -> Response[Any | LightningPaymentData]:
    """Get payment

     View information about the requested lightning payment

    Args:
        store_id (str):
        crypto_code (str):
        payment_hash (str):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[Any | LightningPaymentData]
    """

    kwargs = _get_kwargs(
        store_id=store_id,
        crypto_code=crypto_code,
        payment_hash=payment_hash,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    store_id: str,
    crypto_code: str,
    payment_hash: str,
    *,
    client: AuthenticatedClient,
) -> Any | LightningPaymentData | None:
    """Get payment

     View information about the requested lightning payment

    Args:
        store_id (str):
        crypto_code (str):
        payment_hash (str):

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Any | LightningPaymentData
    """

    return (
        await asyncio_detailed(
            store_id=store_id,
            crypto_code=crypto_code,
            payment_hash=payment_hash,
            client=client,
        )
    ).parsed
